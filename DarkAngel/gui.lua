...
do
	do --frame
		...
		DA_RightClickMenu:SetScript("OnShow",function()
			if InCombatLockdown() then
				...
				if DA_RightClickMenu.calledfrom=="DA_Awarder" and DA_RightClickMenu.player and IsInSameRaidWithMe(DA_RightClickMenu.player) then
	...
	do
		...
		closer99a:SetScript("OnEvent", function(self) if DA_RightClickMenu:IsShown() and DA_RightClickMenu.calledfrom=="DA_Awarder" then self:Click(self) end end)
		closer99a:RegisterEvent("PLAYER_REGEN_DISABLED")

		DA.CreateFFGButton2(nil,DA_RightClickMenu,{"LEFT",DA_RightClickMenu,"TOPRIGHT",-75,-10},14,73,L['Whisper'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_White',{UIDarkAngelFontConsolas:GetFont(), 10, "OUTLINE"},function()
		...
		DA_RightClickMenu.invite=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"LEFT",DA_RightClickMenu,"TOPRIGHT",-75,-25},14,73,L['Invite'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_White',{UIDarkAngelFontConsolas:GetFont(), 9, "OUTLINE"},function()
			DA_RightClickMenu:Hide();
			if DA_RightClickMenu.player then
		...
		DA_RightClickMenu.detailsbtn=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"LEFT",DA_RightClickMenu,"TOPRIGHT",-75,-50},14,73,L['Details'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_White',{UIDarkAngelFontConsolas:GetFont(), 10, "OUTLINE"},function()
			DA_RightClickMenu:Hide();
			if DA_RightClickMenu.player then
		...
		DA_RightClickMenu.epgpaward,DA_RightClickMenu.epgpawardFrame=DA.CreateFFGDropFrame(DA_RightClickMenu,L['award'],14,73,{"LEFT",DA_RightClickMenu,"TOPRIGHT",-75,-80},120,57,"TOPRIGHT",'left')
		DA_RightClickMenu.epgpaward:SetNormalTexture('Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_White')
		DA_RightClickMenu.epgpaward.fs:SetTextColor(0.5,0.9,1,1)
		DA_RightClickMenu.epgpawardFrame.t:SetTexture(0.03, 0.04, 0.07, 0.8)
			do --award
				...
				DA_RightClickMenu.epgpawardFrame.start=DA.CreateFFGButton2(nil,DA_RightClickMenu.epgpawardFrame,{"TOPLEFT", DA_RightClickMenu.epgpawardFrame, "TOPLEFT", 60, -2},13,50,L['add'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_Red',{UIDarkAngelFontConsolas:GetFont(), 9, "OUTLINE"},function(self)
					...
					awardfunc(DA_RightClickMenu.player,string.lower(DA_RightClickMenu.epgpawardFrame.epgp.fs:GetText()),tonumber(value),tostring(reason))
				end,nil,nil)
				...
				local function SetEpgp(epgp,value)
					local btn = DA_RightClickMenu.epgpawardFrame.epgp
					if epgp=='ep' then
						btn:SetText("EP")
				...
				local function gatherFilteredEntries()
					...
					local reason = tostring(DA_RightClickMenu.epgpawardFrame.reason:GetText())
						if reason=="" or reason:gsub("%s+","")=="" then
							reason=nil
						end
					local value = tostring(DA_RightClickMenu.epgpawardFrame.value:GetText())
				...
				function Dropdown_rerender()
					...
					for i=1,20 do
						...
						if not entry then
							...
							DA_RightClickMenu.epgpawardFrame.Dropdown['btn'..i]:SetText(TimeText .. "  |r"..name.. "  |r"..getcoloredval(epgp,value).. " |r("..reason.."|r)")
		...
		DA_RightClickMenu.GKick=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"LEFT",DA_RightClickMenu,"TOPRIGHT",-75,-95},14,73,L['G.Kick'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up_White',{UIDarkAngelFontConsolas:GetFont(), 9, "OUTLINE"},function(self)
		...
		DA_RightClickMenu.assist=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"CENTER",DA_RightClickMenu,"TOPRIGHT",-123.5,-66},15,30,'','Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up',{UIDarkAngelFontConsolas:GetFont(), 9, "OUTLINE"},function(self,butt)
		...
		DA_RightClickMenu.looter=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"CENTER",DA_RightClickMenu,"TOPRIGHT",-92,-66},15,30,'','Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Up',{UIDarkAngelFontConsolas:GetFont(), 9, "OUTLINE"},function(self,butt)
			if UnitIsPartyLeader('player') then
		...
		DA_RightClickMenu.kick=DA.CreateFFGButton2(nil,DA_RightClickMenu,{"CENTER",DA_RightClickMenu,"TOPRIGHT",-116.5,-90},15,44,L['kick'],'Interface\\AddOns\\DarkAngel\\template\\UI-Panel-Button-Highlight',{UIDarkAngelFontConsolas:GetFont(), 9},function()
		...
		do
			...
			copyFrame_Update = function(manual)
				...
				if doing_by_selection then
					for _,player in ipairs(DA_GuildRoster) do
...
if DA_RightClickMenu.calledfrom=="DA_Awarder" and IsInSameRaidWithMe(name) then
...
function DA.RePaintFrames()
	...
	for i,data in ipairs({'DarkAngelGUI.Guild.micromenu','DarkAngelGUI.Guild.bulkmenu','DA_Inviter','DA_Flasker','DA_Flasker.Controls','DA_Flasker.optionsFrame','DA_Awarder','DA_BidTracker'}) do
		...
		-- 	addFrame.t:SetTexture(21/255, 18/255, 22/255, beta)
		-- end
...
-- local zxc = ContentFrame:CreateTexture(nil, "BACKGROUND"); zxc:SetAllPoints(); zxc:SetTexture(8/255, 55/255, 20/255, 0.5); zxc:SetBlendMode("blend")
	...
	for i = 1, NUM_ROWS do
		...
		row:SetScript("OnEnter", function(self)
			...
			if shiftdown and GetMouseFocus() and GetMouseFocus().selfID == self.selfID and self.mydata.plname and self.mydata.officerNoteText then
				DA.myShowTooltip(self, DA.GetTwinsInfo(self.mydata.plname, self.mydata.officerNoteText), {font, 10})
	...
	local function UpdateRows(offset)
		...
		for i = 1, NUM_ROWS do
			local data = DA_GuildRoster[rowIndex + i - 1]
			if data then
